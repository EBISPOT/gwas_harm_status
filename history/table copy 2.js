...
// Optional: Make the fetchDataAndInit function aware of filters
async function fetchDataAndInit() {
  try {
    // Use the filtered URL if filters exist, otherwise use the default URL
    const url = activeFilters.length > 0
      ? `http://127.0.0.1:8000/query?${activeFilters.join('&')}&page=1&size=${pageSize}`
      : `http://127.0.0.1:8000/all_studies?page=1&size=${pageSize}`;
...
// Modify the initializeTable function to include filters
  ...
  dataTable = $('#results-table').DataTable({
    ...
    ajax: async function (data, callback) {
      ...
      try {
        ...
        // Choose the right endpoint based on whether filters exist
        if (activeFilters.length > 0) {
          // Use query endpoint with filters
          url = `http://127.0.0.1:8000/query?${activeFilters.join('&')}&page=${page}&size=${size}`;
        } else {
          // Use all_studies endpoint without filters
          url = `http://127.0.0.1:8000/all_studies?page=${page}&size=${size}`;
        }
        ...
        if (result.total !== undefined) {
          document.getElementById('total-count').innerText = 
            `${result.total.toLocaleString()} summary statistic data`;
        }
        ...
        $('#error-message').text("Failed to load data: " + err.message);
        callback({
          data: [],
          recordsTotal: 0,
          recordsFiltered: 0
...
// Add a clear filters function if needed
function clearFilters() {
  ...
  filterGroups.forEach((group, index) => {
    if (index === 0) {
      // Reset first filter instead of removing it
